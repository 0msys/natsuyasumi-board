...
押すと「ファイルは ほぞんできましたか？」と聞かれます。端末にファイルが入っているのを
確かめてから「ほぞんできた」を押してください。**ここを押すまで、さいごのバックアップの
日づけは変わりません**（共有シートを閉じたときなど、押しただけではファイルが残らないことが
あるためです）。出てこないときは、その場に出る「こちらからほぞん」から保存できます。
...
- **スペシャルチャレンジ** — 宿題を全部やると解放されるボーナス枠（おてつだい・うんどうなど、1つ+25点。点が付くのはせいかつも全部できて100点になった日だけ）
...
画面ごとの現行仕様は [なつやすみボード仕様書](docs/spec/README.md) にまとめています。
...
- どちらかのカテゴリが空（項目0件）だと、そのカテゴリは0点固定になり100点に届きません
  （満点スタンプ・れんぞく満点・スペシャルチャレンジの加点が出なくなるので、管理画面が警告します）
- 「はじめとおわりだけ」の習慣（早寝早起きなど）は、期間の最初と最後の n 日間（既定5日）だけ記録欄が出ます
- 「中止がありうる行事」（ラジオ体操など）は雨天中止の日を「中止」と記録でき、満点扱いになります
- スペシャルチャレンジは、その日の宿題を全部やると解放されます（せいかつは条件に入れません）
- ただし加点は基本点が100点の日だけ。スペシャルチャレンジ1つにつき **+25点** のボーナス
  （解放されていても、せいかつに「やらなかった」や未記入が残る日は点になりません）
