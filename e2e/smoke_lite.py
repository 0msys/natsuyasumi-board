...
確かめること:
  ...
  ⑦ バックアップを取って、読み直しても問いかけが残り、手元にあると答えて、消して、戻せる
     （＝聞きそびれても取り返せるし、消えても取り返せる）
...
async def main() -> int:
    ...
    async with async_playwright() as p:
        ...
        if backup_path is None:
            ...
            # 問いかけは読み直しても残っている。iPhone では、ファイルをさがしているあいだに
            # 共有シートやプレビューから「もどる」でこの画面が作り直される。問いかけを画面の
            # 中だけに持っていたころは、そこで聞く口ごと消えて、ファイルは端末にあるのに
            # 「まだバックアップしていません」が二度と引っ込まなかった。
            # bun test は IndexedDB を使わないので、保存をまたいだ読み直しはここでしか見られない。
            if "ファイルは ほぞんできましたか？" not in await page.inner_text("body"):
                problems.append("⑦ 読み直したら「ほぞんできましたか？」の問いかけが消えている")
