...
export async function replaceAll(raw: unknown): Promise<void> {
	...
	await mutate((db) => {
		...
		// 未回答の問いかけは、こちらの分もファイルの分も落とす（上の local に入れないのは、
		// 据え置くのではなく捨てるため）。基準をいまの通番へ引き直した以上、復元より前に
		// 書き出したファイルはもう受け取れない——api 側が「合わなくなっていた」と断る。
		// 答えても断られるだけの問いかけを、復元したばかりの親の前に置かない。
