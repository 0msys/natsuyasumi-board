...
export type Meta = {
	/**
	 * 記録が変わった書き込みの通番。多タブの取り合いとバックアップの催促に使う。
	 *
	 * 「記録が変わった」がここの肝。保存の持続を聞いた結果やホームの案内を閉じたことも
	 * 1回の書き込みだが、これらでは上げない（$lib/store/db の mutate の local）。
	 * 上げてしまうと、記録が1つも変わっていないのに催促の「そのあと N件」が増える。
	 * とくに navigator.storage.persist が無い端末では persisted が null のままなので、
	 * タブを開くたびに1件ずつ積み上がり、夏休みのあいだ毎日開くだけで警告のしきい値を
	 * 越えてしまう——催促が実態と合わなくなると、保護者は印ごと無視するようになる。
	 *
	 * 数える通番を増やして「記録の変更ぶんだけ」を別に持つ手もあるが、それはやらない。
	 * 新しい欄を知らない版（Service Worker のキャッシュに残った古いタブ、配信の切り戻し）は
	 * この seq しか上げないので、そちらで書いた記録が催促からまるごと消える。この欄を
	 * 増やさずに「上げない書き込みを決める」なら、どの版が書いても数えかたは1つで済む。
	 *
	 * 多タブ側がこれで足りるのは、割り込みを防いでいるのが通番の比較ではなく
	 * 書き込みの鍵と「書く直前の読み直し」だから（値そのものは誰も比べていない）。
	 * 手元の写しを捨てさせる BroadcastChannel の通知は、上げない書き込みでも飛ばす。
	 */
	seq: number;
