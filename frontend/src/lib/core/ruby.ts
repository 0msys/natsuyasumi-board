// ふりがな（ルビ）記法の解析。表示にも採点前の文言生成にも使うのでコア側に置く。
...
//
// 解析規則は backend/app/summer/kanji.py の parse_ruby() と同じ。

/** 基底＝漢字の連続ラン（教育漢字は U+4E00-9FFF に収まるが広めに取る）。 */
export const KANJI = /[㐀-鿿々〆〇ヶ]/;
 ...
 * バックエンドの kanji.ruby_reading() と同じ向き。
 */
...
/**
 * ルビ注記だけを落として基底テキストへ戻す（漢字《よみ》→ 漢字）。
 * stripRuby() とは向きが逆なので取り違えないこと。配当外漢字の検出はこちらを使う。
 * バックエンドの kanji.strip_ruby() と同じ。
 */
export const stripRubyMarkup = (text: string): string =>
	(text ?? '').replace(RUBY_MARKUP, '').replaceAll('｜', '');
