	...
	backupMarkSaved: ({ seq, exported_at, storage_id }: BackupTicket) =>
		...
		mutate(
			(db) => {
				...
				// 覚えるほうを新しいものに寄せる。
				//
				// 寄せる相手が未来なら、それは無かったことにする。いま入っている日づけが
				// 未来になることがある——時計が進んでいた端末で確かめた、その端末で取った
				// バックアップから復元した。抱えこむと、時計が直っても日数が0のまま張りつき、
				// 確かめ直しても正しい（小さい）時刻に上書きされず、催促が二度と出ない。
				//
				// かといって「いま」まで丸めて比べてもいけない。丸めた値は**どのファイルの
				// 時刻でもない**ので、3日前に書き出したファイルを確かめたときに、それより
				// 新しい「きょう」が勝ってしまう——手元にあるのは3日前のものなのに、
				// 催促が3日ぶん遅れる。当てにならない値は、比べる相手から外す。
				const now = nowEpochSec();
				const at = Math.min(exported_at, now);
				const known = db.meta.last_backup_at ?? 0;
				db.meta.last_backup_at = known > now ? at : Math.max(at, known);
