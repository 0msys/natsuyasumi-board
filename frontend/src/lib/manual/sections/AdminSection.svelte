<script lang="ts">
	...
	const TAB_NOTES: Record<SectionId, string> = {
		...
		habits:
			'早起き・歯みがきなどの毎日の習慣。並べ替えができるのはここだけです。「はじめとおわりだけ」（期間の端だけ出す）、「中止」を選べるようにするか、テレビタイマーの上限も、このタブにあります。',
		daily: 'ドリル・日記など、毎日やる宿題。宿題ごとにメモ欄（文字・えらぶ・分と秒）を足せます。',
		challenges:
			'100点の日にだけ開くおまけの項目。1件25点は変えられないので、決めるのは名前だけです。',
		rewards:
...
<ManualSection id="admin">
	...
	<ul>
		...
		<li>
			...
			いまの設定をもとに来年の分を作ります。<strong>学年が1つ上がり、日づけが1年後にずれます</strong
			>。項目と名前は引き継ぎますが、<strong>今年のチェック記録は引き継ぎません</strong
			>。おでかけ予定は空になります（小6の設定からは作れません）
		</li>
		<li>
			<UiLabel><Download size={13} />エクスポート（JSON）</UiLabel> —
			その年の<strong>設定だけ</strong>をファイルに書き出します（チェック記録は入りません）。ファイル名は「年-名前.json」です
		</li>
		<li>
	...
	<ul>
		...
		<li>
			<UiLabel><Upload size={13} />JSON をインポート</UiLabel> — 書き出したファイルを読み込みます。{#if edition === 'lite'}設定1年ぶんのファイルか、端末まるごとのバックアップかは<strong
				>自動で見分けます</strong
			>{:else}設定1年ぶんのファイルを取り込みます{/if}。すでに同じ子ども・同じ年がある場合は取り込めません
		</li>
	</ul>

	<EditionNote only="lite" {edition}>
		<p>
			この画面のいちばん上に<strong>バックアップのカード</strong>が出ます（→
	...
	<h3>その子の設定を直す</h3>
	<p>
		2年ぶん以上を登録している子には、上に<strong>年のタブ</strong>が出ます。右の
		<UiLabel>この年をけす</UiLabel> は、<strong>いま表示している年だけ</strong>を消します。
	</p>
	<p>設定は次の {SECTIONS.length} 枚のタブに分かれています。</p>
	<ul>
		{#each SECTIONS as s (s.id)}
			<li><strong>{s.label}</strong> — {TAB_NOTES[s.id]}</li>
		{/each}
	</ul>

	<h3>直したら「ほぞんする」を押す</h3>
	<p>
		<strong
			>いちばん下の帯にある <UiLabel><Save size={13} />ほぞんする</UiLabel> を押すまで、直した内容は保存されません。</strong
	...
	<ul>
		...
		<li>
			学年でまだ習わない漢字を使うと注意が出ます。ふりがなの付きかたもその場で確認できます
		</li>
		<li>
			記録に使われている項目を消そうとすると、<strong>「何日ぶんの記録が使えなくなるか」</strong
			>を出して確認します。夏休みの途中で項目を足したり消したりすると、過去の日の点数が変わることがあるので、その注意も出ます
		</li>
	...
	<EditionNote only="docker" {edition}>
		<p>
			...
			でその場で確かめられます。VOICEVOX が止まっていると、その旨が出ます。
		</p>
	</EditionNote>
